...
async function testNotificationModule() {
  try {
    ...
    const { notifications, total } = await notificationRepository.findByUserId(testUserId, {
      limit: 5,
      offset: 0
    });
    ...
    notifications.forEach(notification => {
      console.log(`  - ID: ${notification.id}, Type: ${notification.type}, Status: ${notification.status}, Message: ${notification.message.substring(0, 50)}...`);
    });

    // Test 3: Test rate limiting
    console.log('⚡ Testing rate limiting...');
    
    const canSend = await notificationRepository.canSendNotification(testUserId, 10);
    ...
    const { notifications: allNotifications } = await notificationRepository.findAllWithFilters({
      limit: 100,
      offset: 0
    });
